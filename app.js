...
function App() {
  ...
  useEffect(() => {
    ...
    const loadExcelData = async () => {
      try {
        ...
        console.log("Extracted project names from Excel:", extractedProjectNames);
        ...
        console.log("Weights row from Excel:", weightsRow);
        ...
        extractedProjectNames.forEach((projectName, index) => {
          ...
          totalWeight += weight;
          console.log(`Weight for ${projectName}: ${weight}`);
        });
        
        console.log("Total weight:", totalWeight);
        console.log("Initial weights map:", weightsMap);
        ...
        console.log("Valid project names (non-zero weights):", validProjectNames);
        console.log("Setting projectNames to:", validProjectNames);
  ...
  useEffect(() => {
    ...
    if (mcdaMethod === 'weighted_sum') {
      // Weighted sum method
      console.log("Building stacked data for weighted_sum method");
      ...
      stackedData = classData.map(classItem => {
        ...
        projectNames.forEach(projectName => {
          const originalValue = classItem.projectValues[projectName] || 0;
          const weight = weights[projectName] || 0;
          const weightedValue = originalValue * weight;
          row[projectName] = weightedValue;
          
          console.log(`Building data for ${classItem.name} - ${projectName}:`, {
            originalValue,
            weight,
            weightedValue
          });
      ...
      console.log("Final stackedData:", stackedData);
    } else if (mcdaMethod === 'cp') {
    ...
    console.log("D3 stack series:", series);
    console.log("Series keys:", series.map(s => s.key));
    ...
    svg
      ...
      .selectAll("rect")
      .data(d => d.map(item => ({ ...item, seriesKey: d.key })))
      .join("rect")
      ...
      .on("mouseover", function(event, d) {
        // Debug logging
        console.log("Mouseover event data:", d);
        console.log("ClassData:", classData);
        console.log("Weights:", weights);
        console.log("ProjectNames:", projectNames);
        ...
        // Get the correct data for this segment
        const className = d.data.class;
        console.log("D3 stack data structure:", d);
        console.log("d.seriesKey:", d.seriesKey);
        console.log("d.data:", d.data);
        
        // Use the seriesKey to get the project name
        const projectName = d.seriesKey;
        const weight = weights[projectName] || 0;
        // Find the original class data to get the original value
        const originalClassData = classData.find(c => c.name === className);
        const originalValue = originalClassData ? originalClassData.projectValues[projectName] || 0 : 0;
        const weightedValue = d.data[projectName] || 0; // This is already the weighted value
        ...
        console.log("Debug values:", {
          ...
          originalValue,
          weightedValue,
          originalClassData: originalClassData ? originalClassData.projectValues : null
        });
        
        tooltip.html(`
          <strong>${className}</strong><br/>
          <strong>${projectName}</strong><br/>
          Data Value: ${originalValue.toFixed(2)}<br/>
          Weight: ${(weight * 100).toFixed(1)}%<br/>
          Weight × Data: ${weightedValue.toFixed(3)}
        `);
      ...
      if (chartType === 'bar') {
        ...
        svg.append("polygon")
          ...
          .on("mouseover", function(event) {
            ...
            tooltip.html(`
              <strong>Radar Chart Data Area</strong><br/>
              Hover over points or axis labels for detailed values
            `);
